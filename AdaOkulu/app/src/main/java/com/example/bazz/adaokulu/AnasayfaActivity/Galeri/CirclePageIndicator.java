package com.example.bazz.adaokulu.AnasayfaActivity.Galeri;
...
public class CirclePageIndicator extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private void addIndicator(int count) {
        for (int i = 0;  i < count; i++) {
            ...
            img.setImageResource(R.drawable.galeri_circle_icon1);
            addView(img, params);
        }

        if (count > 0) {
            ((ImageView) getChildAt(0)).setImageResource(R.drawable.galeri_circle_icon);
        }
    }

    private void updateIndicator(int position) {
        if (activePosition != position) {
            ((ImageView) getChildAt(activePosition)).setImageResource(R.drawable.galeri_circle_icon1);
            ((ImageView) getChildAt(position)).setImageResource(R.drawable.galeri_circle_icon);
