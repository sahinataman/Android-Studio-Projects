...
import com.example.bazz.adaokulu.OBS.OgrenciProfili;
...
public class LoginPage extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    List<OgrenciSinifi> kullanicilar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        kullanicilar = new ArrayList<OgrenciSinifi>();
        kullanicilar.add(new OgrenciSinifi("Bager Temel","1","1","8.Sinif","123502454"));
        kullanicilar.add(new OgrenciSinifi("Şahin Ataman","shn.ataman","123456","7.Sinif","130401025"));
        kullanicilar.add(new OgrenciSinifi("Serhat Uce","serhat.uce","456789","6.Sinif","130142525"));
        kullanicilar.add(new OgrenciSinifi("Gökhan Özdemir","gkhn73","123123","7.Sinif","120501203"));
        ...
        girisButton.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                if(hata){
                    ...
                    boolean varmı = false;
                    for (OgrenciSinifi k : kullanicilar) {
                        if(k.KullaniciAdi.equals(str_user) && k.Sifre.equals(str_password)){
                            ...
                            pDialog.show();
                            Intent giris = new Intent(getApplicationContext(),OgrenciProfili.class );
                            startActivity(giris);
                            finish();

                            //adSoyad ve Numara yı diger sınıfa gonderdim
                            Intent intent  = new Intent(LoginPage.this,OgrenciProfili.class);
                            intent.putExtra("GelenADSOYAD", k.Adi);
                            intent.putExtra("GelenNumara", k.Numara);
                            intent.putExtra("GelenSinif",k.Sinifi);
                            startActivity(intent);
                        }
                    }
                    ...
                    if(varmı==false){
                        ...
                        alertDialog.setMessage("Girdiğiniz bilgilere dair sistemde kayıtlı öğrenci bulunmamaktadır.");
                        alertDialog.setCancelable(false);
                        alertDialog.setButton(RESULT_OK,"Tamam", new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                password.setText("");
